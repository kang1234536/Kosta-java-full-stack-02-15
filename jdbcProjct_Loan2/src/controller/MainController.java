...
//		loan_type_rate
		...
		while (true) {
			System.out.println(
					"1.고객정보 확인하기||2.대출정보 확인하기||3.대출종류 조회하기||4.대출종류 삽입하기||5.대출종류 수정하기||6.대출종류 삭제하기||7.관리자 로그아웃||8.프로그램종료||!!");
			...
			case 1:
				...
				for (CustomerVO cc : ctvlist) {
					...
					System.out.println("고객아이디:" + c_id + "|고객비밀번호:" + c_pass + "|고객 전화번호:" + c_phone + "|고객이름:" + c_name
							+ "|고객직업:" + c_job + "|고객의 계좌번호:" + c_accountNo + "|");
			...
			case 2:
				...
				for (LoanTextVO ll : ltvlist) {
					...
					l_month_money = (int) Math
							.round((l_money * 10000) * ((double) (l_rate + 100) / 100) / (l_year * 12));
					
					System.out.println(l_cust_id + "번 고객님의 대출정보");
					System.out.println("대출조회번호:" + l_number + "|대출이름:" + l_name + "|대출기간(년단위):" + l_year + "년|대출희망금액:"
							+ l_money + "만원|" + "이자율" + l_rate + "%|총 대출 금액은:" + l_total + "원|매달 납부 금액은:"
							+ l_month_money + "원|||");
				}
				System.out.println("======================================================================");
				break;
			case 3:
				...
				System.out.println("==================================대출종류=======================================================");
				for (Loan_typeVO ltvolist2 : ltvolist) {
					System.out.println("대출이름:"+ltvolist2.getLoan_type_name()+"||대출최소금액:"+ltvolist2.getLoan_type_min()+"||대출최대금액:"+ltvolist2.getLoan_type_max()+"||이자율:"+ltvolist2.getLoan_type_rate()+"||대출실행일:"+ltvolist2.getLoan_type_day()+"||관리자아이디:"+ltvolist2.getManager_id());
				}
				System.out.println("==========================================================================================");
				break;
			case 4:
...
//			}
	...
	private static void Two_Screen(CustomerVO ctv, List<LoanTextVO> ltvlistsize) {
		...
		int id = 0;
		id = ctv.getCust_id();
		ltvlist = ltd.selectByIdCheck(id);

		while (true) {

			System.out.println("1.대출가능여부 확인하기||2.대출금 납부하기||3.대출내역조회||4.자신의 계좌잔액 조회");

			int loan_work = scan.nextInt();

			switch (loan_work) {
			case 1:
				aa: if (ctv.getCust_job().equals("무직") && ltvlist.size() < 1) {
					...
					break;
				} else if (ctv.getCust_job().equals("대학생") && ltvlist.size() < 1) {
					System.out.println("대출이 가능합니다");
					System.out.println("1.대출하기2.이전화면 돌아가기 >>");
					int loan_work1 = scan.nextInt();
					switch (loan_work1) {
					...
					break;
				} else if (ctv.getCust_job().equals("직장인") && ltvlist.size() < 1) {
					System.out.println("대출이 가능합니다");
					System.out.println("1.대출하기2.이전화면 돌아가기 >>");
					int loan_work1 = scan.nextInt();
					switch (loan_work1) {
					...
					break;
				} else if (ctv.getCust_job().equals("주부") && ltvlist.size() < 1) {
					System.out.println("대출이 가능합니다");
					System.out.println("1.대출하기2.이전화면 돌아가기 >>");
					int loan_work1 = scan.nextInt();
					switch (loan_work1) {
			...
			case 2:
				...
				ctvlist = ctd.selectbyID(ctv.getCust_id());
				int c_money = 0;// 고객잔액
				...
				for (CustomerVO clist : ctvlist) {
					c_money = clist.getCust_account_balance();
				}

				for (LoanTextVO list : ltvlist) {
					a = list.getLoanText_money() * 10000;
					b = list.getLoanText_month();
					b1 = list.getLoanText_year() * 12;
				}

				for (Loan_typeVO list1 : ltvolist) {
					c = list1.getLoan_type_rate();
				}
				int d = (int) (a * ((double) (c + 100) / 100) / b1); // 달마다 납부할 금액
				if (a == 0) {
					System.out.println("납부할 대출금은 0원입니다.");
					break;
				} else {
					...
					if (text.equals("y") || text.equals("Y")) {
						if (b > 0) {
							if (c_money >= d) {
								...
								ltd.update(ctv.getCust_id());
								System.out.println("납부하셨습니다.");
								if ((b - 1) == 0) {
									System.out.println("다갚으셨습니다.");
									System.out.println("대출내역을 삭제하겠습니다.");
									ltd.delete(ctv.getCust_id());
								} else {
									System.out.println((b - 1) + "달 남으셨습니다.");
								}
								break;
							} else {
								System.out.println("납부할 금액이 모자랍니다.");

								System.out.println("계좌에 잔액을 넣겠습니까? (Y/N)");
								String ch = scan.next();

								if (ch.equals("y") || ch.equals("Y")) {
									System.out.print("계좌에 넣으실 금액을 말해주세요>>>");
									int u_money = scan.nextInt();
									ctd.updatecustbalance(ctv.getCust_id(), u_money);
									System.out.println(u_money + "원이 입금되었습니다");
									break;
			...
			case 3:
				...
				for (LoanTextVO list : ltvlist) {
					...
					b2_1 = list.getLoanText_year() * 12;
				}
				for (Loan_typeVO list1 : ltvolist) {
					c2 = list1.getLoan_type_rate();
				}
				int d2 = (int) (a2 * ((double) (c2 + 100) / 100) / b2_1); // 달마다 납부할 금액
				...
				if (a2 == 0) {
					System.out.println("대출내역이없습니다.");
					break;
				}
				System.out.println(
						"========================================================================================================================================");
				System.out.println("대출조회번호:" + e + "번|고객이름:" + name + "|대출이름:" + f + "|대출원금:" + a2 / 10000 + "만원|이자율:"
						+ c2 + "%|대출금:" + i + "원|대출기간(1년단위):" + g + "년|고객번호:" + h + "번|달마다 갚아야 하는 금액:" + d2 + "원");
				System.out.println("남은기간(월단위):"+b2+"개월|남은금액:"+(i-d2*(12-b2))+"원");
				System.out.println(
						"========================================================================================================================================");
				break;

			case 4:
				int id_all = ctv.getCust_id();

				ctvlist = ctd.selectbyID(id_all);
				for (CustomerVO ctt : ctvlist) {
					System.out.println("고객의 계좌 잔액:" + ctt.getCust_account_balance());
	...
	private static void Loan_func(int id) {
		...
		for (Loan_typeVO ltvoo : ltvolist) {
			if (ltvoo.getLoan_type_name().equals(name)) {
				System.out.println(name + " 특징:||" + "연이자" + ltvoo.getLoan_type_rate() + "%||최소금액"
						+ ltvoo.getLoan_type_min() + "만원||최대금액" + ltvoo.getLoan_type_max() + "만원|| 대출기간은1년단위|");
				...
				if (money < ltvoo.getLoan_type_min() || money > ltvoo.getLoan_type_max()) {
					...
					int paymonthmoney = (int) Math
							.round(money * 10000 * (double) (ltvoo.getLoan_type_rate() + 100) / 100)
							/ ltvo.getLoanText_month();
